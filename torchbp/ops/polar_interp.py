...
def cart_to_polar(
    ...
    Inverse of :func:`polar_to_cart`. The input is an ordinary Cartesian
    image including the range carrier (as generated by
    :func:`~torchbp.ops.cfbp`, :func:`~torchbp.ops.backprojection_cart_2d` or
    :func:`polar_to_cart`). The interpolation removes the carrier so that the
    output matches a polar image generated with `dealias=True` and range
    modulation `alias_fmod`. The main use is applying algorithms that assume
    a polar grid, such as phase gradient autofocus, to Cartesian format
    images: ``cart_to_polar`` -> process -> :func:`polar_to_cart` back.

    Parameters
    ----------
    img : Tensor
        2D radar image in [x, y] format. Dimensions should match with grid_cart grid.
        [nbatch, x, y] if interpolating multiple images at the same time.
    origin : Tensor
        3D antenna phase center of the new polar image with respect to the input image.
        Units in meters [nbatch, 3] if img shape is 3D.
    grid_cart : CartesianGrid or dict
        Cartesian grid definition. Can be:
...
def _prepare_cart_to_polar_linear_args(
    ...
    """Prepare arguments for C++ cart_to_polar_linear operator.

    Returns tuple of arguments matching C++ operator signature.
    Used internally by cart_to_polar_linear and for testing.
    """
    nbatch = get_batch_dims_img(img, origin)
    x0, x1, y0, y1, nx, ny, dx, dy = unpack_cartesian_grid(grid_cart)
    r0, r1, theta0, theta1, nr, ntheta, dr, dtheta = unpack_polar_grid(grid_polar)
    check_cartesian_grid_matches_img(img, nx, ny)

    return (img, origin, nbatch, rotation, fc, x0, y0, dx, dy,
            nx, ny, r0, dr, theta0, dtheta, nr, ntheta, alias_fmod)


def cart_to_polar_linear(
    img: Tensor,
    origin: Tensor,
    grid_cart: "CartesianGrid | dict",
    grid_polar: "PolarGrid | dict",
    ...
    Inverse of :func:`polar_to_cart_linear`. The input image should include
    the range carrier, the output is dealiased. See :func:`cart_to_polar`.
    ...
    out : Tensor
        Interpolated radar image in [range, angle] format.
    """
    cpp_args = _prepare_cart_to_polar_linear_args(
        img, origin, grid_cart, grid_polar, fc, rotation, alias_fmod
    )
    return torch.ops.torchbp.cart_to_polar_linear.default(*cpp_args)


def cart_to_polar_lanczos(
    img: Tensor,
    origin: Tensor,
    grid_cart: "CartesianGrid | dict",
    grid_polar: "PolarGrid | dict",
    ...
    Inverse of :func:`polar_to_cart_lanczos`. The input image should include
    the range carrier, the output is dealiased. See :func:`cart_to_polar`.
    ...
    return torch.ops.torchbp.cart_to_polar_lanczos.default(
        img,
        origin,
        nbatch,
        rotation,
...
@torch.library.register_fake("torchbp::cart_to_polar_linear")
def _fake_cart_to_polar_linear(
    img: Tensor,
    origin: Tensor,
    nbatch: int,
    rotation: float,
    ...
    torch._check(origin.dtype == torch.float32)
    torch._check(img.dtype == torch.complex64)
    return torch.empty((nbatch, nr, ntheta), dtype=torch.complex64, device=img.device)


@torch.library.register_fake("torchbp::cart_to_polar_linear_grad")
def _fake_cart_to_polar_linear_grad(
    grad: Tensor,
    img: Tensor,
    origin: Tensor,
    nbatch: int,
...
def _backward_cart_to_polar_linear(ctx, grad):
    img = ctx.saved_tensors[0]
    origin = ctx.saved_tensors[1]
    ret = torch.ops.torchbp.cart_to_polar_linear_grad.default(
        grad, img, origin, *ctx.saved
    )
    grads = [None] * cart_to_polar_linear_args
    grads[0] = ret[0]
    grads[1] = ret[1]
    return tuple(grads)


def _setup_context_cart_to_polar_linear(ctx, inputs, output):
    img, origin, *rest = inputs
    for i in range(len(ctx.needs_input_grad)):
        if ctx.needs_input_grad[i]:
            if i <= 1:
                continue
            raise NotImplementedError("Only img and origin gradient supported")
    ctx.saved = rest
    ctx.save_for_backward(img, origin)


torch.library.register_autograd(
    "torchbp::cart_to_polar_linear",
    _backward_cart_to_polar_linear,
    setup_context=_setup_context_cart_to_polar_linear,
)
