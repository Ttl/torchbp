...
TORCH_LIBRARY(torchbp, m) {
  ...
  // Fused backprojection + W1/W2 illumination moment maps (CPU only): the
  // antenna path accumulates the moments anyway, so the fused ops return
  // them instead of recomputing with compute_illumination.
  m.def("backprojection_polar_2d_illum(Tensor data, Tensor pos, Tensor att, int nbatch, int sweep_samples, int nsweeps, float fc, float r_res, float r0, float dr, float theta0, float dtheta, int Nr, int Ntheta, float d0, int dealias, float z0, Tensor g, float g_az0, float g_el0, float g_daz, float g_del, int g_naz, int g_nel, float data_fmod, float alias_fmod, bool normalize, Tensor dem) -> Tensor[]");
  m.def("backprojection_polar_2d_knab_illum(Tensor data, Tensor pos, Tensor att, int nbatch, int sweep_samples, int nsweeps, float fc, float r_res, float r0, float dr, float theta0, float dtheta, int Nr, int Ntheta, float d0, int dealias, float z0, int order, float oversample, Tensor g, float g_az0, float g_el0, float g_daz, float g_del, int g_naz, int g_nel, float data_fmod, float alias_fmod, bool normalize, Tensor dem) -> Tensor[]");
